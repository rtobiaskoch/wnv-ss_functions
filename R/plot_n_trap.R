...
#'
#' @importFrom dplyr filter group_by ungroup mutate summarise n_distinct case_when
...
plot_n_trap <- function(df,
  ...
  df |>
    ...
    ggplot2::ggplot(ggplot2::aes(.data$week, .data$n,
                                 color = .data$trap_status,
                                 fill  = .data$trap_status)) +
    ggplot2::geom_col(alpha = 0.7) +
    ggplot2::scale_fill_manual(values = wnvSurv::trap_status_colors,
                               na.value = "grey80") +
    ggplot2::scale_color_manual(values = wnvSurv::trap_status_colors,
                                na.value = "grey80") +
